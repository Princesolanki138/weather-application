...
//https://api.openweathermap.org/data/2.5/forecast?q=mumbai&appid=1dca1eb96b6c76cfb5428f728827ed24&cnt=56
...
export default function Home() {
  ...
  if (isPending) return <LoaderCompo />;
      ...
      <main className="px-3 max-w-7xl mx-auto flex flex-col gap-9 w-full pb-10 pt-4">
        ...
        <section className="space-y-4">
          <div className="space-y-2">
            ...
            <Container className="bg-cyan-200 gap-10 px-6 items-center">
              {/* teperature */}
              <div className="flex flex-col px-4">
                ...
                <p className="text-sm space-x-1 whitespace-nowrap">
                  ...
                  <span>
                    {convertKelvinToCelsius(data?.list[0].main.feels_like)}
                  </span>
                </p>
                <p className="text-xs space-x-1">
                  <span>
                    {convertKelvinToCelsius(data?.list[0].main.temp_min)}°⬇️{" "}
                  </span>
                  <span>
                    {convertKelvinToCelsius(data?.list[0].main.temp_max)}°⬆️{" "}
                  </span>
                </p>
              </div>
              {/*Time and weather icon */}
              <div className="flex gap-10 sm:gap-16 overflow-x-auto w-full justify-between pr-4">
                {data?.list.map((d, i) => (
                  <div
                    key={i}
                    className="flex flex-col justify-center gap-2 item-center text-center font-semibold"
                  >
          ...
          <div className="flex gap-4 md:flex-row">
            {/* left */}
            <Container className="bg-cyan-200 w-full md:w-fit justify-center flex-col px-4 items-center">
            ...
            <Container className="bg-emerald-300 px-6 gap-4 justify-between overflow-x-auto">
              <WeatherDetails visability={metersToKilometers(data?.list[0].visibility ?? 1000)}
              humidity={`${data?.list[0].main.humidity}%`}
              windSpeed={convertWindSpeed(data?.list[0].wind.speed ?? 1.64)}
        ...
        <section className="flex w-full flex-col gap-4">
          ...
          visability={metersToKilometers(d?.visibility ?? 1000)}
